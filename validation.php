...
class validation
{
    ...
    public function validateUserAlphaNumericInput($data)
    {
        $error = array();
        $validInmput = "#[^a-zA-Z0-9;\\\.,:/]#";

        if (is_array($data)) {
            foreach ($data as $key => $value) {
                $data_arr = explode(" ", $value);
                foreach ($data_arr as $keys => $strings) {
                    if (preg_match($validInmput, $strings)) {
            ...
            foreach ($data_arr as $keys => $strings) {
                if (preg_match($validInmput, $strings)) {
                    $error[] =  "Only alphanumeric characters required for input! Please check your inputs! \n";
                }
            }
        }
